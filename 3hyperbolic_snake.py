#!/usr/bin/python3
# NOTE Comments are still in python2
# Check carefully before uncommenting
import sys, pygame
import math
...
def move_point(p, d, angle):
    ...
    if test < 1.0:
        ...
        p.theta += 0
    new_angle = sign1 * math.acos(
        (math.cosh(p.r) - (math.cosh(new_r) * math.cosh(d))) / (math.sinh(new_r) * math.sinh(d)))
    p.r = new_r
